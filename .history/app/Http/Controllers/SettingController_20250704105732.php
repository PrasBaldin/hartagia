...
class SettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //     if (is_array($setting->partner_logos)) {
        //         foreach ($setting->partner_logos as $oldLogo) {
        //             if (!empty($oldLogo) && file_exists(base_path($oldLogo))) {
        ...
        //     $paths = [];
        //     foreach ($request->file('partner_logos') as $index => $file) {
        //         $filename = time() . "_partner_{$index}_" . str_random(10) . '.' . $file->getClientOriginalExtension();
        //         $file->move($folder, $filename);
        //         $paths[] = 'images/site_assets/' . $filename;
        //     }
