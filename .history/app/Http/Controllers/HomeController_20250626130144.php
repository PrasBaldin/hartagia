...
class HomeController extends Controller
{
    ...
    public function portfolio()
    {
        ...
        // other portfolios
        $otherPortfolios = Portfolio::with(['translations' => function ($query) use ($locale) {
            $query->where('language', $locale);
        }])->where('slug', '!=', $portfolios->first()->slug)->latest()->take(3)->get();

        return view('pages.portfolios.index', compact('portfolios', 'otherPortfolios'));
    }
