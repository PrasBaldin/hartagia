    ...
    public function index()
    {
        ...
        $visitorsThisWeek = DB::table('visitors')
            ...
            ->groupBy(function ($item) {
                return Carbon::parse($item->visited_at)->toDateString(); // Kelompokkan per tanggal
            })
            ->map(function ($group) {
                return $group->map(function ($item) {
                    return $item->ip_address . '|' . $item->user_agent;
                })->unique()->count(); // Hitung unik per hari
            })
            ->sum(); // Jumlahkan semua hari
        ...
        $weeklyStats = $weeklyStats->map(function ($item) {
            return [
                'date' => $item->date,
        ...
        // Hitung total pengunjung per hari    
