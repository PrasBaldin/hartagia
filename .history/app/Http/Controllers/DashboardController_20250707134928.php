...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Mengisi hari yang tidak ada data dengan 0
        $filledWeeklyStats = collect();
        $period = \Carbon\CarbonPeriod::create($startOfWeek, $endOfWeek);

        foreach ($period as $date) {
            $dateStr = $date->toDateString();
            $stat = $weeklyStats->filter(function ($item) use ($dateStr) {
                return $item->date === $dateStr;
            })->first();

            $filledWeeklyStats->push([
                'date' => $dateStr,
        ...
        return view('admin.dashboard', compact(
            ...
            'visitorsThisMonth',
            'monthlyStats'
        ))->with('weeklyStats', $filledWeeklyStats);
    }
}
