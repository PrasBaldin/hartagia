...
class PortfolioController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Upload semua gambar image_1 sampai image_5
            for ($i = 1; $i <= 5; $i++) {
                $key = 'image_' . $i;
                if ($request->hasFile($key)) {
                    $filename = time() . "_{$i}_" . Str::random(16) . '.webp';
                    $images[$key] = $request->file($key)->storeAs('portfolio_images', $filename, 'public');
                } else {
                    $images[$key] = null;
                }
            }
            ...
            // Hapus semua gambar yang sempat terupload
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            foreach ($columns as $column => $langs) {
                ...
                return redirect()->route('portfolio.show', $portfolio->id)->with('success', 'Portofolio berhasil diperbarui!');
            }
        } catch (\Exception $e) {
            DB::rollBack(); // Rollback transaksi jika terjadi error
