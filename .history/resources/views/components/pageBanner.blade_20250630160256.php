        ...
        <div class="md:w-1/2 mb-6 md:mb-0">
            <div class="flex items-center h-full">
                <div class="flex items-center text-lg md:text-2xl text-gray-300 space-x-2 py-10" aria-label="Breadcrumb">
                    <a href="{{ url(app()->getLocale() . '/') }}" class="hover:underline hover:text-white" @aos('fade-left', 200)>
                        {{ __('menu.home') }}
                    </a>
                    ...
                    @if (request()->segment(2) && request()->segment(2) !== app()->getLocale())
                        pan class="text-white font-semibold" @aos('fade-left', 400)>
                        {{ isset($current) ? $current : __('menu.about') }}
                        </span>
                        <span class="text-white font-semibold" @aos('fade-left', 500)>
                            /
                            {{ request()->segment(2) }}
                        </span>
                    @else
