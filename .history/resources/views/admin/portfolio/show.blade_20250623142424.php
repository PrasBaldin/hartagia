                ...
                <div class="flex flex-row gap-6 justify-between items-center mt-4 scrollbar-x overflow-x-auto">
                    @for ($i = 1; $i <= 5; $i++)
                        ...
                        @if (!empty($portfolio->image_2))
                            <div class="max-w-[200px] max-h-[100px] rounded-lg bg-gray-200 overflow-hidden">
                                @if (!empty($portfolio->$key))
                                    <img src="{{ asset('storage/' . $portfolio->$key) }}" alt="image_{{ $i }}.jpg" class="w-full object-cover">
                                @endif
                            </div>
                        @endif
                    @endfor
