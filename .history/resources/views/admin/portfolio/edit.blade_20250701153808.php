    ...
    <div class="flex flex-col justify-center items-center">
        <div class="w-full bg-white backdrop-blur-md rounded-lg shadow-lg border px-4 py-8 relative overflow-hidden">
            ...
            <form action="{{ route('admin.portfolio.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-2 gap-6 p-6">
                ...
                <!-- Input Gambar -->
                <div class="mb-6">
                    <label for="image_1" class="block mb-2 text-base font-semibold">Gambar Utama</label>
                    @if ($portfolio->image_1)
                        <p class="mt-2 text-sm">Gambar saat ini:</p>
                        <img src="{{ asset($portfolio->image_1) }}" alt="Gambar Portofolio" class="mt-2 mb-4 w-40 h-auto rounded-lg">
                    @endif
                    <p class="my-2 text-sm">Ubah gambar:</p>
                    <input type="file" name="image_1" id="image_1" class="w-full border rounded-lg p-3 cursor-pointer">
                </div>
                ...
                @for ($i = 2; $i <= 5; $i++)
                    <div class="mb-6">
                        ...
                        @if (!empty($portfolio->$field))
                            <img src="{{ asset($portfolio->$field) }}" alt="Gambar Tambahan {{ $i - 1 }}" class="mt-2 mb-4 w-40 min-h-[90px] rounded-lg">
                        @else
                            <p class="text-gray-500 my-[45px]">Tidak ada gambar saat ini.</p>
                        @endif
                        <p class="my-2 text-sm">Ubah gambar:</p>
                        <input type="file" name="image_{{ $i }}" id="image_{{ $i }}" class="w-full border rounded-lg p-3 cursor-pointer">
                    </div>
                @endfor
