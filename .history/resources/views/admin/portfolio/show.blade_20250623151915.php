    ...
    <div class="flex flex-col md:flex-row justify-center items-start gap-6">
        <div class="p-8 bg-white rounded-lg shadow-md w-3/4">
            <div class="flex flex-col mb-6">
                <!-- image utama -->
                <div>
                    <img id="mainImage" src="{{ asset('storage/' . $portfolio->image_1) }}" alt="main_image.jpg" class="w-full object-cover rounded-lg transition duration-300">
                </div>
                <!-- image tambahan -->
                <div class="flex flex-row gap-4 justify-start items-center mt-4 scrollbar-x overflow-x-auto">
                    @for ($i = 1; $i <= 5; $i++)
                        ...
                        @if (!empty($portfolio->image_2))
                            <div class="flex-shrink-0 w-auto h-24 max-w-[165px] lg:h-28 mb-4 rounded-lg overflow-hidden bg-gray-100 shadow">
                                @if (!empty($portfolio->$key))
                                    <img src="{{ asset('storage/' . $portfolio->$key) }}" alt="image_{{ $i }}.jpg" class="w-full h-full object-cover transition hover:scale-105 duration-300 border-4 border-transparent" onclick="document.getElementById('mainImage').src = this.src">
                                @endif
                            </div>
                        @endif
                    @endfor
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    // Set default aktif: gambar pertama
                    ...
                    thumbnails.forEach(container => {
                        ...
                        container.addEventListener('click', () => {
                            // Ganti source utama
                            mainImage.src = container.dataset.img;
