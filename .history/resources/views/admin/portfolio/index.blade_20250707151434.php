...
@section('content')
    <div class="p-8 bg-white rounded-lg shadow-md">
        ...
        @else
            <table class="w-full border border-gray-200 rounded-lg border-separate border-spacing-0 overflow-x-auto">
                ...
                <tbody>
                    @foreach ($portfolios as $index => $portfolio)
                        <tr class="{{ $index % 2 === 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200 border-t border-gray-300">
                            <td class="text-center border border-gray-300">{{ $portfolios->firstItem() + $index }}</td>
                            <td class="border border-gray-300">
                                @if ($portfolio->image_1)
                                    <img src="{{ asset($portfolio->image_1) }}" alt="{{ $portfolio->image_1 }}" class="w-full" loading="lazy">
                                @else
                                    Tidak ada gambar
                                @endif
                            </td>
                            <td class="border border-gray-300 p-4">{{ $portfolio->project_name ? $portfolio->project_name->translated_text : '-' }}</td>
                            <td class="border border-gray-300 p-4 hidden md:table-cell">{{ $portfolio->project_desc ? $portfolio->project_desc->translated_text : '-' }}</td>
                            <td class="border border-gray-300 p-4">
                                <div class="flex flex-col gap-2">
                                    <a href="{{ route('admin.portfolio.show', $portfolio->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-4 rounded-lg flex items-center gap-2 transition duration-300">
