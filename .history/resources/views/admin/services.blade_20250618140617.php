...
<div class="p-8 bg-white rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('admin.services.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white hover:text-gray-100 font-semibold py-2 px-4 rounded-lg flex items-center gap-2 transition duration-300">Tambah Layanan</a>
    </div>

    @include('components.alerts')

    <table class="w-full border border-gray-200 rounded-lg shadow-md border-separate border-spacing-0">
        ...
        <tbody>
            ...
            <tr class="{{ $index % 2 === 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200 border-t border-gray-300">
                ...
                <td class="border border-gray-300">
                    @if($service->image_1)
                    <img src="{{ asset('storage/' . $service->image_1) }}" alt="{{ $service->image_1 }}" class="w-full">
                    @else
                    Tidak ada gambar
                    @endif
                </td>
                <td class="border border-gray-300 p-4">{{ $service->project_name ? $service->project_name->translated_text : '-' }}</td>
                <td class="border border-gray-300 p-4">{{ $service->project_desc ? $service->project_desc->translated_text : '-' }}</td>
                <td class="border border-gray-300 p-4">
                    <div class="flex flex-col gap-2">
                        ...
                        <a href="{{ route('admin.services.edit', $service->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white text-sm font-semibold py-2 px-4 rounded-lg flex items-center gap-2 transition duration-300">
                            <i class="fas fa-edit"></i> Edit
                        </a>

                        <form id="delete-form-{{ $service->id }}" action="{{ route('admin.services.delete', $service->id) }}" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
