...
<div class="container mt-4">
    ...
    @if(empty($portfolios))
    <p class="text-red-500">Data portfolios kosong!</p>
    @else
    {{ dump($portfolios) }} <!-- Debug semua data yang dikirim -->
    @endif
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            @php
            $translation = $portfolio->translations->where('language', 'id')->first();
            @endphp
            <tr>
                <td>{{ $index + 1 }}</td>
                <td>
                ...
                <td>{{ $translation ? $translation->project_name_id : '-' }}</td>
                <td>{{ $translation ? $translation->project_desc_id : '-' }}</td>
                <td>
