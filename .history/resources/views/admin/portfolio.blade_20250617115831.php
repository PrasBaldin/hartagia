    ...
    <div class="w-full max-w-2xl bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl border border-green-100 p-10 relative overflow-hidden">
        ...
        <form action="{{ route('portfolio.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div>
                ...
                <input name="description_id" id="description_id" placeholder="Deskripsi (Indonesia)" required
                    class="w-full px-5 py-3 border border-green-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400 transition bg-green-50/50 shadow-inner resize-none min-h-[80px]"></input>
            </div>
            <div>
                <label class="block mb-2 text-base font-semibold text-green-700" for="description_en">Description (English)</label>
                <input name="description_en" id="description_en" placeholder="Description (English)" required
                    class="w-full px-5 py-3 border border-green-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400 transition bg-green-50/50 shadow-inner resize-none min-h-[80px]"></input>
            </div>
            <div>
                <label class="block mb-2 text-base font-semibold text-green-700" for="content_id">Konten (Indonesia)</label>
                <textarea name="content_id" id="content_id" placeholder="Konten (Indonesia)" required
            ...
            <div>
                <label class="block mb-2 text-base font-semibold text-green-700" for="content_en">Content (English)</label>
                <textarea name="content_en" id="content_en" placeholder="Content (English)" required
                    class="w-full px-5 py-3 border border-green-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400 transition bg-green-50/50 shadow-inner resize-none min-h-[80px]"></textarea>
            </div>
            <div>
