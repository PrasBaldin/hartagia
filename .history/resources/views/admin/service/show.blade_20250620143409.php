...
<div class="p-8 bg-white rounded-lg shadow-md">
    ...
    <div class="w-1/4">
        <div class="flex flex-col gap-4">
            ...
            <form action="{{ route('service.destroy', $service->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus layanan ini?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 hover:bg-red-700 text-white text-sm font-semibold py-2 px-4 rounded-lg flex items-center gap-2 transition duration-300">
                    Hapus
                </button>
            </form>
        </div>
