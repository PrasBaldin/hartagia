...
@section('content')
        ...
        <div class="container">
            ...
            <div class="flex flex-col md:flex-row gap-20 justify-start pt-10 md:mt-10">
                <div id="aboutContent" class="w-full md:w-1/2">
                    <p class="mb-10" @aos('fade-up', 400, '#aboutContent')>
                        {{ __('about.description.1') }}
                    </p>
                    <p class="mb-10" @aos('fade-up', 500, '#aboutContent')>
    ...
    <section class="py-20 bg-white relative">
        <div class="container mx-auto">
            <div id="portfolioHeader" class="flex flex-col md:flex-row gap-4 justify-between items-center py-10 mb-10">
                ...
                <div class="w-full md:w-1/2">
                    ...
                    <a href="{{ route('portfolio', ['lang' => app()->getLocale()]) }}" class="btn button-secondary hover:scale-105 transition-transform duration-300" @aos('fade-up', 500, '#portfolioHeader', 200)>
                ...
                @foreach ($portfolios as $index => $portfolio)
                    ...
                    <div class="group relative overflow-hidden rounded-xl shadow-lg cursor-pointer" @aos('fade-up', $delay)>
                        <a href="{{ route('portfolio.show', ['lang' => app()->getLocale(), 'slug' => $portfolio->slug]) }}" class="block">
                            <img src="{{ asset($portfolio->image_1) }}" alt="{{ $caption ? $caption->translated_text : 'Untitled' }}" class="w-full h-80 object-cover transition-transform duration-500 group-hover:scale-110 group-hover:brightness-75">
                            ...
                            <div class="absolute inset-0 hidden md:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/40">
                                <span class="text-white text-xl font-semibold tracking-wide">
                                    {{ $caption ? $caption->translated_text : 'Untitled' }}
                                </span>
                            </div>
        ...
        <!-- Decorative shapes -->
        <div class="hidden xl:!block absolute top-20 left-[40px] w-32 h-32 bg-green-800/60 rounded-full z-10" @aos('fade-', 100)></div>
        <div class="hidden xl:!block absolute top-10 left-10 w-20 h-20 bg-green-600/40 rounded-full z-10" @aos('fade-', 100)></div>
        <div class="hidden xl:!block absolute bottom-20 right-[40px] w-40 h-40 bg-green-900/60 rounded-full z-10" @aos('fade-', 100)></div>
        <div class="hidden xl:!block absolute bottom-10 right-10 w-24 h-24 bg-green-700/40 rounded-full z-10" @aos('fade-', 100)></div>
    </section>
@endsection
