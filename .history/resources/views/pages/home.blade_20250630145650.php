...
@section('content')
            ...
            <div class="md:w-1/2 mb-6 md:mb-0 ">
                <div class="flex items-center h-full">
                    <div class="pt-10 pb-20">
                        <h1 class="lg:text-6xl md:text-4xl text-4xl font-bold leading-tight" @aos('fade-up', 200, '#heroSection')>
                            {{ __('home.headline') }}
                        </h1>
                        <p class="mt-4 tracking-widest" @aos('fade-up', 300, '#heroSection')>{{ __('home.subheadline') }}</p>
                        <div class="mt-6">
                            <a href="{{ url(app()->getLocale() . '/contact') }}" class="btn button-primary hover:scale-105 transition-transform duration-300" @aos('fade-up', 400, '#heroSection')>
                                {{ __('contact.button') }}
                            </a>
                            <a href="https://katalog.inaproc.id/harta-gemilang-aselindo" class="btn button-secondary bg-transparent hover:bg-white/80 text-white hover:text-green-700 border-2 border-white hover:border-green-700 ml-4 hover:scale-105 transition-transform duration-300" @aos('fade-up', 500, '#heroSection')>
        ...
        <div class="container">
            <div id="serviceHeader" class="flex flex-col md:flex-row gap-4 justify-between items-center py-10 mb-10">
                <div class="w-full md:w-1/2">
                    <h2 class="uppercase tracking-widest text-green-700" @aos('fade-up', 200, '#serviceHeader', 200)>{{ __('service.title') }}</h2>
                    <p class="text-6xl mt-4 text-gray-900 font-bold" @aos('fade-up', 300, '#serviceHeader', 200)>{{ __('service.subtitle') }}</p>
                </div>
                <div class="w-full md:w-1/2">
                    <p class="mb-10 text-gray-700" @aos('fade-up', 400, '#serviceHeader', 200)>{{ __('service.description') }}</p>
                    <a href="{{ route('services', ['lang' => app()->getLocale()]) }}" class="btn button-secondary inline-flex items-center gap-2 hover:scale-105 transition-transform duration-300" @aos('fade-up', 500, '#serviceHeader', 200)>
            ...
            @php
                ...
                $baseDelay = 100;
                $resetInterval = $gridCols; // Reset delay setiap baris
                $delay = 0;
            @endphp
                ...
                @foreach ($services as $index => $service)
                    ...
                    <a href="{{ route('service.show', ['lang' => $lang, 'slug' => $service->slug]) }}" class="group relative block rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300" @aos('fade-up', $delay)>
                        ...
                        <div class="absolute bottom-0 left-0 p-6">
                            <h3 class="text-2xl font-semibold text-white drop-shadow-[0_1px_4px_rgba(0,0,0,1)]">{{ $caption->translated_text ? $caption->translated_text : 'Untitled' }}</h3>
                            <p class="mt-2 text-white hidden drop-shadow-[0_1px_4px_rgba(0,0,0,1)] group-hover:block transition-all duration-300">{{ $service->service_desc ? $service->service_desc->translated_text : 'Null' }}</p>
                        </div>
                        <span class="absolute top-4 right-4 bg-green-700 text-white px-3 py-1 rounded-full text-xs font-bold shadow">{{ $index + 1 }}</span>
                    </a>
                @endforeach
            </div>
        </div>
    ...
    <section class="py-10 bg-gray-100">
        <div class="container">
            <div id="aboutHeader" class="w-full md:w-3/4">
                <h2 class="uppercase tracking-widest text-green-700" @aos('fade-up', 200, '#aboutHeader', 200)>{{ __('about.title') }}</h2>
                <p class="text-2xl md:text-4xl mt-4 font-bold" @aos('fade-up', 300, '#aboutHeader', 200)>{{ __('about.subtitle') }}</p>
            </div>
            <div class="flex flex-col md:flex-row gap-20 justify-start pt-10 md:mt-10">
                <div id="aboutContent" class="w-full md:w-1/2">
                    <p class="mb-10" @aos('fade-up', 400, '#aboutContent')>{{ __('about.description.1') }}</p>
                    <p class="mb-10" @aos('fade-up', 500, '#aboutContent')>{{ __('about.description.2') }}</p>
                    <a href="{{ route('about', ['lang' => app()->getLocale()]) }}" class="btn button-secondary hover:scale-105 transition-transform duration-300" @aos('fade-up', 600, '#aboutContent')>
                        {{ __('about.learn_more') }}
                    </a>
                </div>
    ...
    <section class="bg-gradient-to-br from-green-900 to-green-700 text-white py-20">
        <div class="container mx-auto py-10">
            <h1 class="text-center text-2xl md:text-4xl font-bold mb-8" @aos('fade-up', 200)>{{ __('about.benefits.title') }}</h1>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                <div class="p-6 rounded-lg">
                    <div class="mb-4" @aos('fade-up', 400)>
                        <i class="fas fa-check-circle text-4xl"></i>
                    </div>
                    <h2 class="text-2xl lg:text-4xl font-semibold mb-4" @aos('fade-up', 500)>{{ __('about.benefits.reliability_title') }}</h2>
                    <p class="pb-4" @aos('fade-up', 600)>{{ __('about.benefits.reliability_desc') }}</p>
                </div>
                <div class="p-6 rounded-lg">
                    <div class="mb-4" @aos('fade-up', 800)>
                        <i class="fas fa-lightbulb text-4xl"></i>
                    </div>
                    <h2 class="text-2xl lg:text-4xl font-semibold mb-4" @aos('fade-up', 900)>{{ __('about.benefits.innovation_title') }}</h2>
                    <p class="pb-4" @aos('fade-up', 1000)>{{ __('about.benefits.innovation_desc') }}</p>
                </div>
                <div class="p-6 rounded-lg">
                    <div class="mb-4" @aos('fade-up', 1200)>
                        <i class="fas fa-user-tie text-4xl"></i>
                    </div>
                    <h2 class="text-2xl lg:text-4xl font-semibold mb-4" @aos('fade-up', 1300)>{{ __('about.benefits.professional_title') }}</h2>
                    <p class="pb-4" @aos('fade-up', 1400)>{{ __('about.benefits.professional_desc') }}</p>
                </div>
            </div>
        </div>
    </section>
                ...
                @foreach ($portfolios as $portfolio)
                    <div class="group relative overflow-hidden rounded-xl shadow-lg cursor-pointer">
                        <a href="{{ route('portfolio.show', ['lang' => app()->getLocale(), 'slug' => $portfolio->slug]) }}" class="block">
                            <img src="{{ asset($portfolio->image_1) }}" alt="{{ $portfolio->project_name ? $portfolio->project_name->translated_text : 'Untitled' }}" class="w-full h-80 object-cover transition-transform duration-500 group-hover:scale-110 group-hover:brightness-75">
                            ...
                            <div class="absolute inset-0 hidden md:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/40">
                                <span class="text-white text-xl font-semibold tracking-wide">
                                    {{ $portfolio->project_name ? $portfolio->project_name->translated_text : 'Untitled' }}
                                </span>
                            </div>
