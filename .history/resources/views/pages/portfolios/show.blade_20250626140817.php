                            ...
                            @foreach ($otherPortfolios as $otherPortfolio)
                                <div class="group relative overflow-hidden rounded-xl shadow-lg cursor-pointer">
                                    <a href="{{ route('portfolio.show', ['lang' => app()->getLocale(), 'slug' => $otherPortfolio->slug]) }}" class="block">
                                        <img src="{{ asset('storage/' . $otherPortfolio->image_1) }}" alt="{{ $otherPortfolio->project_name ? $otherPortfolio->project_name->translated_text : 'Untitled' }}" class="w-full h-80 object-cover transition-transform duration-500 group-hover:scale-110 group-hover:brightness-75">
