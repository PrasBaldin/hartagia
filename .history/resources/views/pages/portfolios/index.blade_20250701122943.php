...
@section('content')
    ...
    <section>
        <div class="container mx-auto py-12 pb-20">
            <h1 class="py-20 md:w-3/4 text-4xl lg:text-6xl pr-20 font-bold leading-tight" @aos('fade-up', 200)>
                {{ __('portfolio.headline') }}
            </h1>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($portfolios as $index => $portfolio)
                    @php
                        $delay = 100 + $index * 100; // Delay bertambah 100ms untuk setiap portfolio
                        $anchor = '#portfolio' . $index; // Anchor untuk AOS
                    @endphp
                    <div id="portfolio{{ $index }}">
                        <div class="bg-white rounded-lg shadow-lg overflow-hidden transition-shadow duration-300 hover:shadow-2xl group" @aos('fade-up', 200 + $index * 100, '#portfolioGrid', 250)>
