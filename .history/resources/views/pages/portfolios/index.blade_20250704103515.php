                ...
                @foreach ($portfolios as $index => $portfolio)
                    ...
                    <div id="portfolio{{ $index }}">
                        <div class="bg-white rounded-lg shadow-lg overflow-hidden transition-shadow duration-300 hover:shadow-2xl group" @aos('fade-up', $delay, $anchor, 250)>
                            ...
                            <div class="p-6 min-h-[226px]">
                                <h3 class="text-xl font-semibold mb-2 text-gray-900">
                                    {{ $portfolio->project_name ? $portfolio->project_name->translated_text : 'value' }}
                                </h3>
                                <p class="text-gray-700 mb-4">
                                    {{ $portfolio->project_desc ? $portfolio->project_desc->translated_text : 'value' }}
                                </p>
                                <a href="{{ route('portfolio.show', ['lang' => app()->getLocale(), 'slug' => $portfolio->slug]) }}" class="absolute bottom-6 btn button-primary mt-6 hover:scale-105 transition-transform duration-300">
