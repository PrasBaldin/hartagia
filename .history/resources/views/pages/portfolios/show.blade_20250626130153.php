        ...
        <div class="container">
            <div class="flex flex-col gap-8">
                <div class="flex flex-col md:flex-row gap-8">
                    ...
                    <div class="w-full md:w-1/3 p-8 bg-white rounded-lg shadow-md">
                        <h3 class="text-2xl font-semibold mb-4">{{ __('portfolio.other_portfolios') }}</h3>
                        <div class="flex flex-col gap-6">
                            @foreach ($otherPortfolios as $otherPortfolio)
                                <a href="{{ route('portfolio.show', ['lang' => app()->getLocale(), 'slug' => $otherPortfolio->slug]) }}" class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition-shadow duration-300">
                                    <div class="w-full h-48 overflow-hidden rounded mb-4">
                                        <img src="{{ asset('storage/' . $otherPortfolio->image_1) }}" alt="{{ $otherPortfolio->project_name ? $otherPortfolio->project_name->translated_text : 'title' }}" class="w-full h-48 object-cover hover:scale-105 transition-transform duration-300">
                                    </div>
