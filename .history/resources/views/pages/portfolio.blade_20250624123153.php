            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($portfolios as $portfolio)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden transition-shadow duration-300 hover:shadow-2xl group">
                        <a href="#">
                            <img src="{{ asset('storage/' . $portfolio->image_1) }}" alt="{{ $portfolio->project_name ? ->value }}" class="w-full h-[200px] object-cover transition-transform duration-300 group-hover:scale-110">
                        </a>
                        <div class="p-6">
                            <h3 class="text-xl font-semibold mb-2 text-gray-900">{{ $portfolio->project_name?->value }}</h3>
                            <p class="text-gray-700 mb-4">{{ $portfolio->project_desc?->value }}</p>
