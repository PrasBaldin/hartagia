                        ...
                        <div class="flex flex-col gap-6">
                            @foreach ($otherServices as $otherService)
                                <a href="{{ route('service.show', ['lang' => app()->getLocale(), 'slug' => $otherService->slug]) }}" class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition-shadow duration-300">
                                    <img src="{{ asset('storage/' . $otherService->image) }}" alt="{{ $otherService->service_name ? $otherService->service_name->translated_text : 'title' }}" class="w-full h-48 object-cover rounded mb-4 hover:scale-105 transition-transform duration-300">
