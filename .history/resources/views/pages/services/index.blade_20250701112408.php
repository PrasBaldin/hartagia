                    ...
                    @foreach ($services as $index => $service)
                        ...
                        <div id="service{{ $index }}" class="flex flex-col md:flex-row items-center gap-6 py-10">
                            ...
                            <div class="flex-1 p-6">
                                ...
                                <p class="mt-2" @aos('fade-up', $delay, '#service' . $index)>
                                ...
                                <ul class="list-disc pl-5 mt-4 space-y-2">
                                    @for ($i = 1; $i <= 3; $i++)
                                        @php
                                            ...
                                            $delayPoint = $delay + 200 + ($i - 1) * 100; // Delay bertambah 100ms untuk setiap poin
                                        @endphp
                                        @if ($point)
                                            <li @aos('fade-up', $delayPoint, '#service' . $index)>
                                                {{ $point->translated_text }}
                                            </li>
                                        @endif
                                    @endfor
                                </ul>
                                <a href="{{ route('service.show', ['lang' => app()->getLocale(), 'slug' => $service->slug]) }}" class="btn button-secondary mt-6 items-center gap-2 hover:scale-105 transition-transform duration-300" @aos('fade-up', $delay, '#service' . $index)>
                                    <span>
