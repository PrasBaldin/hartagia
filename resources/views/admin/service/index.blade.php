                        ...
                        @foreach ($services as $index => $service)
                            <tr class="{{ $index % 2 === 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200 border-t border-gray-300">
                                ...
                                <td class="border border-gray-300">
                                    @if ($service->image)
                                        <img src="{{ asset($service->image) }}" alt="{{ $service->image }}" class="w-full">
                                    @else
                                        Tidak ada gambar
                                    @endif
                                </td>
                                ...
                                <td class="border border-gray-300 p-4">
                                    <div class="flex flex-col gap-2">
                                        <a href="{{ route('admin.service.show', $service->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-4 rounded-lg flex items-center gap-2 transition duration-300">
